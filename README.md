...
If we want to stream the log generated by the dyno we can create a log session that will connect to the log stream of the specified dyno and stream the result back to us, which is specified [here](https://devcenter.heroku.com/articles/platform-api-reference#log-session-create) in the Heroku API reference.
