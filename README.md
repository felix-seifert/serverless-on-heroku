...
## Retreving log session

When we have invoked the command on Heroku, the API will respond with a JSON object containing the name of the dyno in which our command is being executed.

If we want to stream the log generated by the dyno we can create a log session that will connect to the log stream of the specified dyno and stream the result back to us, which is specified [here](https://devcenter.heroku.com/articles/platform-api-reference#log-session-create) in the Heroku API reference. We include the parameters `"source": "app"` and `"tail": true`" to specify that we only want the logs generated from the command itself and that it should stream the ongoing logs.
...
The response from creating the log session will include a URL to the log session, specified under the key `logplex_url`, this URL can be open in you browser as it does not require additional authorization. Or fetch it using CURL.
